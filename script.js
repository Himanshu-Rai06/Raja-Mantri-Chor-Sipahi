...
function renderGame(data) {
    ...
    switch (state) {
        ...
        case 'PICKING': {
            ...
            statusDiv.textContent = myChit ? "Waiting for others to pick…" : "🖐 Grab a Parchi!";
            break;
        }
        case 'REVEAL': {
        ...
        case 'GUESSING': {
            ...
            } else if (amIMantriPlayer(data)) {
                // Shouldn't happen but safe fallback
                statusDiv.textContent = "Mantri is guessing…";
            } else {
                statusDiv.textContent = "Mantri is guessing…";
            }
            break;
...
function renderChits(data) {
    ...
    data.chits.forEach((chit, chitIdx) => {
        ...
        if (chit.pickedBy !== undefined && chit.pickedBy !== -1) {
            ...
            // When to show role text:
            const showRole =
                isMyChit                                                   ||
                isFullReveal                                               ||
                (state === 'REVEAL' && role === 'RAJA')                    ||
                (state === 'GUESSING' && (role === 'RAJA' || role === 'MANTRI'));
    ...
    if (state === 'GUESSING' && amIMantri()) {
        data.players.forEach((p, idx) => {
            ...
            slot.onclick = () => openGuessModal(idx, p.name, roles[heldChitIdx]);
        });
    }
}
...
function checkAllPicked() {
    ...
    setTimeout(() => {
        ...
        get(child(ref(db), `rooms/${roomCode}/chits`)).then((snapshot) => {
            ...
            // Count how many players are expected
            const playerCount = currentGameState.players
                ? currentGameState.players.length
                : 4;
...
// Close scoreboard when clicking the overlay (on mobile)
